...
CLASS_ASPECT_RATIOS = {
    _CLASSNAMES[0]: 1 / 1.223,
    _CLASSNAMES[1]: 2.0,
    _CLASSNAMES[2]: 1 / 1.667,
    _CLASSNAMES[3]: 1 / 1.445,
    _CLASSNAMES[4]: 0.5,
}
...
class MVTecLocoDataset(torch.utils.data.Dataset):
    ...
    def __init__(
        ...
        ratio = CLASS_ASPECT_RATIOS.get(classname, 1.0)
        total_pixels = resize**2
        w = round(np.sqrt(total_pixels / ratio))
        h = round(ratio * w)

        self.resize = [h, w]
        self.imagesize = (3, round(h/32)*32, round(w/32)*32)
