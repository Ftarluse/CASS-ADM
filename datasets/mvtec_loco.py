...
from datasets.perlin import perlin_mask
...
class CenterCrop32:
    ...
    def __call__(self, img):
        ...
        # 如果原始尺寸小于32，则使用原始尺寸
        new_width = max(new_width, 32) if width >= 32 else width
        new_height = max(new_height, 32) if height >= 32 else height
        ...
        # 中心裁剪
        return transforms.CenterCrop((new_height, new_width))(img)
...
class MVTecLocoDataset(torch.utils.data.Dataset):
    ...
    def __init__(
            self,
            source,
            anomaly_source_path='D:/Dataset/dtd/images',
            dataset_name='mvtec',
            classname='leather',
            resize=288,
            imagesize=288,
        ...
        if classname == _CLASSNAMES[0]:
            self.imagesize = (3, self.imgsize, int(1.223 * self.imgsize))
        elif classname == _CLASSNAMES[1]:
            self.imagesize = (3, int(2 * self.imgsize), self.imgsize)
        elif classname == _CLASSNAMES[2]:
            self.imagesize = (3, self.imgsize, int(1.667 * self.imgsize))
        elif classname == _CLASSNAMES[3]:
            self.imagesize = (3, self.imgsize, int(1.445 * self.imgsize))
        elif classname == _CLASSNAMES[4]:
            self.imagesize = (3, self.imgsize, int(2 * self.imgsize))
        ...
        if anomaly_source_path in ("", None, "None", "none"):
    ...
    def rand_augmenter(self):
        ...
        transform_aug = [
            transforms.Resize(self.imagesize[1:]),
            list_aug[aug_idx[0]],
            list_aug[aug_idx[1]],
            list_aug[aug_idx[2]],
            CenterCrop32(),
    ...
    def __getitem__(self, idx):
        ...
        if self.split == DatasetSplit.TRAIN:
            if self.anomaly_source_paths is not None:
                ...
                mask_all = perlin_mask(image.shape, (self.imagesize[1] // self.downsampling,
                                                     self.imagesize[2] // self.downsampling), 0, 6, mask_fg, 1)
                ...
                aug_image = mask_s = False

        if self.split == DatasetSplit.TEST and mask_path is not None:

            mask_gt = PIL.Image.open(os.path.join(mask_path, "000.png")).convert('L')
