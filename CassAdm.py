...
class CassAdm(torch.nn.Module):
    ...
    def set_model_dir(self, model_dir, dataset_name):
        ...
        self.best_roc_pt = os.path.join(self.weights_dir, "best_roc.pth")
    ...
    def test(self, training_data, test_data, test, save_segmentation_images, evaluate_current_model):
        ...
        with tqdm.tqdm(total=len(test_data)) as tq:
            ...
            for data in test_data:
                ...
                aggregator["mask"].append(data["mask_gt"].cpu().numpy())
        ...
        if evaluate_current_model:
            ...
            aupro = round(metrics.compute_pro(mask, segmentations, num_th=200), 4)
        ...
        with tqdm.tqdm(total=self.gan_epochs * len(input_data)) as tq:
            ...
            for i_epoch in range(self.gan_epochs):
                ...
                for data_item in input_data:
                    ...
                    self.exist_aug = 0 if data_item["aug"].dtype == torch.bool else 1
                    ...
                    if self.pre_proj > 0:
                        ...
                        if self.exist_aug > 0:
                            aug_feats = self.pre_projection(self.extranet(aug))
                    else:
                        true_feats = self.extranet(img)
                        if self.exist_aug > 0:
                            aug_feats = self.extranet(aug)

                    if self.add_pos > 0:
                        true_feats = self.add_position(true_feats)
                        if self.exist_aug > 0:
                            aug_feats = self.add_position(aug_feats)
                    ...
                    for step in range(self.max_steps):
                        ...
                        bin_true_scores, bin_fake_scores = self.sigmoid(true_scores), self.sigmoid(fake_scores)
                        ...
                        if (step + 1) % self.training_steps == 0 or fin_st:
                            ...
                            if self.exist_aug > 0:
                                ...
                                aug_scores = self.sigmoid(self.discriminator(aug_feats))

                                aug_dist = (aug_scores - mask_s_gt) ** 2
                                d_hard = torch.quantile(aug_dist, q=0.5)

                                aug_scores_ = aug_scores[aug_dist >= d_hard].unsqueeze(1)
                                mask_ = mask_s_gt[aug_dist >= d_hard].unsqueeze(1)

                                output = torch.cat([1 - aug_scores_, aug_scores_], dim=1)
                                aug_loss = self.focal_loss(output, mask_)

                                loss = true_loss.mean() + fake_loss.mean() + aug_loss.mean()
                            else:
                    ...
                    p_true = (1 - bin_true_scores.detach()).sum() / len(true_scores)
                    p_fake = bin_fake_scores.sum() / len(fake_scores)
    ...
    def load_state_dicts(self, checkpoint):
        ...
        self.exist_aug = checkpoint["exist_aug"]
    ...
    def _predict_dataloader(self, dataloader):
        ...
        with tqdm.tqdm(total=len(dataloader)) as tq:
            ...
            if self.best_record == (None, None, None):
                ...
                torch.save(self.checkpoint, self.best_roc_pt)
                head, color = "FIRST TRAIN" + " " * (len(head) - 11), "green"
            ...
            if i_auroc + p_auroc > self.best_record[0] + self.best_record[1]:
                self.best_record = [i_auroc, p_auroc, aupro]
                self.checkpoint["best_record"] = self.best_record
                torch.save(self.checkpoint, self.best_roc_pt)
    ...
    def _predict(self, images):
        ...
        with torch.no_grad():
            ...
            output = self.discriminator(features).reshape(-1, h, w)

            if self.exist_aug > 0:
                patch_scores = image_scores = self.sigmoid(output)
            else:
                patch_scores = image_scores = output
