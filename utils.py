...
def plot_segmentation_images(
    ...
    for image_path, mask_path, anomaly_score, segmentation in tqdm.tqdm(
        ...
        if save_type > 0:
            ...
            # bmp supported
            filename, ext = os.path.splitext(savename)
            savename = f"{filename}.png"
            ...
            save_clean_image(segmentation, os.path.join(savename, 'segmentation_magma.png'), cmap='magma')
            save_clean_image(segmentation, os.path.join(savename, 'segmentation_jet.png'), cmap='jet')
...
def save_clean_image(img, save_path, cmap=None):
    if cmap is not None:
        ...
        img_color = (cmap_func(img)[..., :3] * 255).astype(np.uint8)  # RGBA -> RGB
