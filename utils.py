...
#         # 应用 colormap（使用 matplotlib 获取 RGB）
...
def create_tabel(results_path):
    ...
    data = {"class": [], "epoch": [], "image_auroc": [], "pixel_auroc": []}
    for c in class_list:
        try:
        ...
        try:
            data["epoch"].append(int(r["epoch"].iloc[-1]))
        except  IndexError:
            continue

        max_iroc = r.sort_values(by=["image_auroc", "epoch"], ascending=[False, False]).iloc[0]
        max_proc = r.sort_values(by=["pixel_auroc", "epoch"], ascending=[False, False]).iloc[0]

        iroc = (round(max_iroc["image_auroc"], 4).item(), int(max_iroc["epoch"]))
        proc = (round(max_proc["pixel_auroc"], 4).item(), int(max_proc["epoch"]))
    ...
    data["epoch"].append(int(sum(data["epoch"]) / l))

    data["image_auroc"].append(round(sum([i[0] for i in data["image_auroc"]]) / l, 4))
    data["pixel_auroc"].append(round(sum([i[0] for i in data["pixel_auroc"]]) / l, 4))
