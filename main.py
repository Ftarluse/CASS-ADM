...
@click.option("--evaluate_current_model",  type=int, default=0, show_default=True)
@click.option("--save_segmentation_images",  type=int, default=0, show_default=True)
@click.option("--save_type",  type=int, default=1, show_default=True)
...
def run(
    ...
    for dataloader_count, dataloaders in enumerate(list_of_dataloaders):
        ...
        for i, CassAdm in enumerate(CassAdm_list):
            # training or testing
            mode = 1 if test in ("", "0", None) else 0
            ...
            if mode > 0:
                LOGGER.info("Training models ({}/{})".format(i + 1, len(CassAdm_list)))
                i_auroc, p_auroc, pro_auroc = CassAdm.train(dataloaders["training"], dataloaders["testing"])
            else:
                CassAdm.save_type = save_type
                LOGGER.info("Testing models ({}/{})".format(i + 1, len(CassAdm_list)))
                i_auroc, p_auroc, pro_auroc = CassAdm.test(dataloaders["training"], dataloaders["testing"], test, save_segmentation_images, evaluate_current_model)
            ...
            if mode > 0 or evaluate_current_model > 0:
                for key, item in result_collect[-1].items():
                    if key != "dataset_name":
                        LOGGER.info("{0}: {1:3.3f}".format(key, item))
            else:
                LOGGER.info("Model not evaluated, no accuracy metrics")
            LOGGER.info("\n\n-----\n")

            if mode > 0:
                utils.create_tabel(run_save_path)
            elif evaluate_current_model > 0:
...
@click.argument("aug_path", type=str)
